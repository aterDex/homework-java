...
class HeraldInjectorParametersLogBySystemOutTest {
    ...
    void injectWithoutParameters(Type returnType) {
        ...
        order.verify(mockMethodVisitor).visitFieldInsn(anyInt(), any(), any(), any());
        order.verify(mockMethodVisitor).visitLdcInsn(anyString());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), any(), eq("(Ljava/lang/String;)V"), eq(false));
    ...
    void injectWithOneParameters(Type returnType) {
        ...
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), any(), eq("(Ljava/lang/Object;)V"), eq(false));
        order.verifyNoMoreInteractions();
    }
    ...
    @ParameterizedTest
    @MethodSource({"returnTypes"})
    void injectWithLotOfParameterAndVariableMetaButWithoutPrintPropertyWitnComment(Type returnType) {
        AtomicInteger counter = new AtomicInteger(1);
        AtomicInteger counterWithShift = new AtomicInteger(1);
        HeraldMeta meta = HeraldMeta.builder()
                .printProperty(false)
        ...
        bySystemOut.inject(meta, mockMethodVisitor);
        order.verify(mockMethodVisitor).visitFieldInsn(anyInt(), any(), any(), any());
        order.verify(mockMethodVisitor).visitInsn(anyInt());
        order.verify(mockMethodVisitor).visitLdcInsn(anyString());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("print"), eq("(Ljava/lang/String;)V"), eq(false));
        order.verify(mockMethodVisitor).visitLdcInsn(eq(" // Test comment"));
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("println"), eq("(Ljava/lang/String;)V"), eq(false));
        order.verifyNoMoreInteractions();
    }

    private void checkInit(InOrder order) {
        order.verify(mockMethodVisitor).visitTypeInsn(anyInt(), any());
        order.verify(mockMethodVisitor).visitInsn(anyInt());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), any(), any(), anyBoolean());
    }

    private void checkConst(InOrder order) {
        order.verify(mockMethodVisitor).visitLdcInsn(anyString());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("append"), any(), anyBoolean());
    }

    private void checkVar(InOrder order) {
        order.verify(mockMethodVisitor).visitVarInsn(anyInt(), anyInt());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("append"), any(), anyBoolean());
    }

    private void checkVar(InOrder order, String descriptor) {
        order.verify(mockMethodVisitor).visitVarInsn(anyInt(), anyInt());
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("append"), eq(descriptor), anyBoolean());
    }

    private void checkConst(InOrder order, String cons) {
        order.verify(mockMethodVisitor).visitLdcInsn(eq(cons));
        order.verify(mockMethodVisitor).visitMethodInsn(anyInt(), any(), eq("append"), any(), anyBoolean());
    }
}